...
import { Flex, HStack, Link, Text } from "../ui";
	...
	return (
		<div className="bg-background ">
			<div className="relative">
				...
				<main className="mb-8 border-b border-b-foreground">{children}</main>
			...
			>
				{categories.data.map((category) => (
					<Text.Small key={category._id} className="text-foreground">
						<Link.Nav to={`/categories/${category._slug}`}>{category.name}</Link.Nav>
					</Text.Small>
				))}
			</HStack>
		</header>
